 ...
 */
    (email, log, record, runtime, search) => {
        ...
        const getInputData = (inputContext) => {
            ...
            return search.create({
                ...
                filters: [
                    ...
                    ["mainline", "is", "T"],
                    'AND',
                    ["status", "anyof", "SalesOrd:A"] // For Pending Approval
                ],
                columns: [
                    'internalid',
                    'entity', // Customer
        ...
        const reduce = (reduceContext) => {
            ...
            let salesOrderIdList = [];
            reduceContext.values.forEach(function (record) {
                console.log("dataid:" + record.id)
                let data = JSON.parse(record);
                salesOrderIdList.push(data.id);

                // Update the Sales Order status to "Pending Fulfillment"
                record.submitFields({
            ...
            reduceContext.write({
                ...
                value: JSON.stringify(salesOrderIdList)
            });
        ...
        const summarize = (summaryContext) => {
            ...
            summaryContext.output.iterator().each(function (salesRep, salesOrderIdList) {
                let recipientEmail;
                let message;

                if (salesRep === 'no_salesrep') {
                    recipientEmail = 'adminfun043024LP@netsuite.com';
                    message = 'The following sales orders have been updated to "Pending Fulfillment":\n\n';
                } else {
                    recipientEmail = getSalesRepEmail(salesRep);
                    message = 'The following sales orders for your customers have been updated to "Pending Fulfillment":\n\n';
                }

                log.debug("recipientEmail: " + recipientEmail)

                JSON.parse(salesOrderIdList).forEach(function (salesOrderId) {
                    message += `Sales Order ID: ${salesOrderId}\n`;
                });

                email.send({
                    author: -5, // Replace with the internal ID of the sender
